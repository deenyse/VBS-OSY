...
//***************************************************************************
// Function to process a batch of expressions using Python
void process_batch(int t_sock_client, std::vector<std::string> &expressions)
{
    if (expressions.empty())
        return;

    log_msg(LOG_INFO, "Processing a batch of %zu expressions.", expressions.size());

    // Construct a Python script as a string from the received expressions
    std::string py_cmd;
    for (const auto &expr : expressions)
    {
        // For an input like "4+5", this creates a line: print("4+5=", 4+5)
    ...
    if (py_pid == 0)
    {
        // Grandchild process: executes python3
        ...
        dup2(t_sock_client, STDOUT_FILENO); // Redirect stdout to the client socket
    ...
    // Child process (the client handler): writes to the pipe and waits for the grandchild
    close(pipe_fd[0]); // Close the read-end of the pipe
    // Write the Python command string to the pipe
    write(pipe_fd[1], py_cmd.c_str(), py_cmd.length());
    close(pipe_fd[1]); // Close the write-end, signaling EOF to the reader (python3)

    // Wait for the grandchild to finish to avoid creating a zombie process
    waitpid(py_pid, NULL, 0);

    // Clear the buffer of expressions after they have been processed
    expressions.clear();
}
...
void handle_client(int t_sock_client)
{
    ...
    // Buffer to accumulate expressions from the client for batch processing
    ...
    while (1)
    {
        ...
        if (l_read_poll[0].revents & POLLIN)
        {
            ...
            // Add the received expression to the buffer
            expressions.push_back(std::string(l_buf));

            // If we have collected 3 or more expressions, process them as a batch
            if (expressions.size() >= 3)
            {
                process_batch(t_sock_client, expressions);
            }
        }
    }
